# ============================================================
#  Fast Food, Obesity & Poverty in America — Shiny App v2
#  Data: USDA Food Environment Atlas, BLS, Census SAIPE/ACS
# ============================================================
...
atlas <- read.csv("atlas_wide.csv", stringsAsFactors = FALSE) %>%
  mutate(
    # FIX: formatC properly zero-pads FIPS (sprintf %05s only space-pads strings,
    # causing CA/AZ/CO/AR counties starting with 0 to fail joins)
...
# State-level data (real: BLS unemployment + Census ACS poverty + median income)
...
# Clean named lookup (avoids the fragile unlist/names pattern that caused bar chart crash)
bar_var_labels <- c(
...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem("state",
        fluidRow(
          box(width=3, title="State Map Controls", status="warning", solidHeader=TRUE,
            ...
            hr(),
            p(class="src",
              strong("Poverty:")," Census ACS 2019–2023, families below poverty.",br(),br(),
              strong("Unemployment:")," BLS LAUS, annual averages.",br(),br(),
              strong("Income:")," Census ACS 2022, median household.")
          ),
          box(width=9, title="State-Level Map", status="warning", solidHeader=TRUE,
            leafletOutput("state_leaf",height=480))
        ),
        fluidRow(
          box(width=6, title="State Poverty Rankings — Family Poverty (ACS 2019–2023)",
              status="warning", solidHeader=TRUE,
            plotlyOutput("state_pov_bar",height=370)),
          box(width=6, title="Unemployment Rate 2019–2023 by State (BLS)",
              status="warning", solidHeader=TRUE,
            p(class="src","Select states below to highlight (leave blank = show all)"),
            selectInput("unemp_filter","Filter states:",
                        choices=c("",sort(state_data$State)), multiple=TRUE),
      ...
      tabItem("scatter",
        fluidRow(
          box(width=3, title="Scatter Controls", status="primary", solidHeader=TRUE,
            selectInput("sc_x","X Axis:", choices=scatter_vars, selected="POVRATE21"),
      ...
      tabItem("cpi",
        fluidRow(
          box(width=3, title="CPI Controls", status="info", solidHeader=TRUE,
            ...
            p(style="font-size:0.84em; color:#444;",
              "Since 2020, restaurant / fast-food prices have inflated",
              " much faster than grocery prices, squeezing low-income",
              " households that rely on affordable food options."),
            hr(),
      ...
      tabItem("demo",
        fluidRow(
          box(width=3, title="Controls", status="danger", solidHeader=TRUE,
            ...
            hr(),
            p(style="font-size:0.84em; color:#444;",
              "Each panel plots counties by racial/ethnic share",
              " vs. the selected outcome. Lines show trend direction.",
              " Each dot = one county."),
            hr(),
            p(class="src","Race: Census 2020 via Food Env. Atlas."),
...
server <- function(input, output, session) {
  ...
  # Helper: look up label from code
  ...
  output$map_leaf <- renderLeaflet({
    ...
    pal <- colorNumeric(palette=input$map_palette, domain=vals,
                        na.color="#cccccc", reverse=input$map_reverse)
    ...
    leaflet(data) %>%
      ...
      addPolygons(fillColor=~pal(vals), fillOpacity=0.8,
                  color="#555", weight=0.3, smoothFactor=0.5,
                  highlight=highlightOptions(weight=2,color="#222",
                                             fillOpacity=0.95,bringToFront=TRUE),
                  label=labels,
                  labelOptions=labelOptions(
                    style=list("font-weight"="normal",padding="3px 6px"),
                    textsize="13px", direction="auto")) %>%
      addLegend(pal=pal, values=vals, title=var_label, position="bottomright",
                labFormat=if(is_dollar) labelFormat(prefix="$",big.mark=",",digits=0)
                          else labelFormat(digits=1)) %>%
  ...
  output$state_leaf <- renderLeaflet({
    ...
    pal <- colorNumeric(palette="YlOrRd", domain=vals,
                        na.color="#cccccc", reverse=is_dollar)
    ...
    leaflet(data) %>%
      addProviderTiles("CartoDB.Positron") %>%
      addPolygons(fillColor=~pal(vals), fillOpacity=0.8,
                  color="#555", weight=0.5, smoothFactor=0.5,
                  highlight=highlightOptions(weight=2.5,color="#222",
                                             fillOpacity=0.95,bringToFront=TRUE),
                  label=labels,
                  labelOptions=labelOptions(
      ...
      addLegend(pal=pal, values=vals, title=var_label, position="bottomright",
                labFormat=if(is_dollar) labelFormat(prefix="$",big.mark=",",digits=0)
                          else labelFormat(suffix="%",digits=1)) %>%
      setView(lng=-96, lat=38, zoom=4)
  })
  ...
  output$state_pov_bar <- renderPlotly({
    ...
    p <- ggplot(d, aes(x=State, y=PovertyPct_FamilyACS,
      ...
      labs(y="Family Poverty Rate (%)", x=NULL,
           caption="Source: Census ACS 2019–2023 via HDPulse/NIMHD") +
  ...
  output$unemp_trend <- renderPlotly({
    ...
    if (highlighted) d <- d %>% filter(State %in% filter_st)

    p <- ggplot(d, aes(x=Year, y=UnempRate, group=StateAbbr, color=StateAbbr,
                        text=paste0(State," (",Year,"): ",UnempRate,"%"))) +
      geom_line(linewidth=1, alpha=if(highlighted) 0.9 else 0.4) +
      geom_point(size=1.8) +
      ...
      theme(legend.position=if(highlighted) "right" else "none",
            plot.caption=element_text(color="#999",size=8))

    ggplotly(p, tooltip="text") %>%
      layout(showlegend=highlighted)
  })
  ...
  output$scatter_plot <- renderPlotly({
    ...
    # Always pull METRO23 so coloring works without re-filtering
    d <- atlas %>%
    ...
    if (input$sc_smooth) {
      p <- p + geom_smooth(
        data=d, aes(x=.data[[xvar]], y=.data[[yvar]]),
        method="lm", se=TRUE,
        color="#2c3e50", fill="#2c3e50",
  ...
  output$bar_plot <- renderPlotly({
    ...
    p <- ggplot(state_agg, aes(x=State, y=val, fill=val,
    ...
    if (input$bar_refline && nrow(state_agg) > 0) {
      ...
      } else paste0("US avg: ",round(us_avg,1))
  ...
  output$cpi_plot <- renderPlotly({
    ...
    p <- ggplot(d, aes(x=Year, y=Pct, color=Label, group=Label,
  ...
  output$cpi_cumulative <- renderPlotly({
    ...
    p <- ggplot(d, aes(x=Year, y=Cumul, color=Label, group=Label,
  ...
  output$demo_plot <- renderPlotly({
    ...
    clr <- c("% Black"="#e74c3c","% Hispanic"="#f39c12",
             "% White (Non-Hisp)"="#3498db","% Asian"="#2ecc71")
