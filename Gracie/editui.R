...
ui <- fluidPage(
  ...
  navbarPage(title="", id="nav",
    ...
    tabPanel("Chains",
      ...
      tabsetPanel(
        tabPanel("Chain Cards", br(), uiOutput("chainCards")),
        ...
        tabPanel("Full Table", br(), DTOutput("chainsTable"))
      )
    ),
    ...
    tabPanel("Nutrition",
      ...
      tabsetPanel(
        ...
        tabPanel("Item Explorer", br(),
          fluidRow(
            column(3,
              div(class="filter-panel",
                div(class="chart-box-title","Filter Items"),
                br(),
                selectInput("nutrRestaurant","Restaurant",choices=NULL,multiple=TRUE),
                sliderInput("nutrCal","Max Calories",200,2430,2430,step=50),
                sliderInput("nutrSod","Max Sodium (mg)",0,6000,6000,step=100)
              )
            ),
            column(9,
              div(class="chart-box",
                div(class="chart-box-title","Menu Item Detail Table"),
                div(class="chart-box-sub","Full item-level nutrition data"),
                DTOutput("nutrTable")
              )
            )
          )
    ...
    tabPanel("Demographics",
      div(class="section-header",
        div(div(class="section-label","Demographics"),
            div(class="section-title","Race & Ethnicity by State")),
        div(class="section-meta","Source: KFF State Health Facts · 2024")
      ),
      fluidRow(
        column(5, div(class="chart-box",
          div(class="chart-box-title","U.S. National Racial Composition"),
          div(class="chart-box-sub","Population share by race/ethnicity"),
          plotlyOutput("donutChart", height="340px"))),
        column(7, div(class="chart-box",
          div(class="chart-box-title","Fast Food Locations by State — Top 20"),
          div(class="chart-box-sub","Count of locations in dataset"),
          plotlyOutput("ffStateChart", height="340px")))
      ),
      div(class="chart-box",
        div(class="chart-box-title","Racial Composition — Stacked Bars by State"),
        div(class="chart-box-sub","White · Black · Hispanic · Asian (2024 estimates)"),
        plotlyOutput("raceStackChart", height="620px")
      )
    ),
            ...
            div(style="display:flex;align-items:center;gap:.6rem;",
              ...
              selectInput("mapOverlay", NULL,
                choices=c("None (Chain Colors)"="none",
                          "Obesity Rate by State"="obesity",
                          "Poverty Rate by State"="poverty"),
                width="220px")
            ),
